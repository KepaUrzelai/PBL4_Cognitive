...
public class PuntuacionesBaja extends Puntuaciones{

	public PuntuacionesBaja(int memoria, int agilidadMental, int atencionDirecto, int atencionInverso,
	...
	public PuntuacionesBaja(int atencion, int memoria, int agilidad, int razmat, int razverb, String fecha) {
	super(atencion, memoria, agilidad, razmat, razverb, fecha);
	}
	
	public PuntuacionesBaja(int atencion, int memoria, int agilidad, int razmat, int razverb) {
	...
	public void calcularBaremos() {
			...
			else if(this.atencionDirecto<=5) dAtencion=1;
			else if(this.atencionDirecto<=7) dAtencion=2;
			else dAtencion=3;
			if(this.atencionInverso<=3) iAtencion=0;
			else if(this.atencionInverso<=4) iAtencion=1;
			...
			if(this.razVerbalAnimales<=9) rAnimales=0;
			else if(this.razVerbalAnimales<=12) rAnimales=1;
			else if(this.razVerbalAnimales<=19) rAnimales=2;
			else rAnimales=3;			
			if(this.razVerbalAbstraccion<=2) rAbstraccion=0;
			else if(this.razVerbalAbstraccion<=5) rAbstraccion=1;
			...
			else if(this.memoria<=3) cMemoria=1;
			else if(this.memoria<=5) cMemoria=2;
			else cMemoria=3;
			
			if(this.agilidadMental<=18) cAgilidad=0;
			else if(this.agilidadMental<=19) cAgilidad=1;
			else if(this.agilidadMental<=20) cAgilidad=2;
			else cAgilidad=3;
			
			int mCalculos, mProblemas;
			if(this.razMatCalculos<=4) mCalculos=0;
			else if(this.razMatCalculos<=6) mCalculos=1;
			else if(this.razMatCalculos<=22) mCalculos=2;
			else mCalculos=3;
			if(this.razMatProblemas<=2) mProblemas=0;
			else if(this.razMatProblemas<=5) mProblemas=1;
			else if(this.razMatProblemas<=12) mProblemas=2;
			else mProblemas=3;
